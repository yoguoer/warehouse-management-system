<template>
  <div class="list-model">
    <el-table :data="tableData" :cell-style="{ padding: '5px' }"
      :header-cell-style="{ background: '#F2F6FC', color: '#606266' }" v-loading="loading" tooltip-effect="dark"
      :height="height" style=" width: auto;margin-top: 20px" @selection-change="handleSelectionChange" border>
      ...
      <template v-for="(rowItem, rowIdx) in tableColumn">
        <template v-if="rowItem.children">
          <el-table-column :label="rowItem.label" :key="(rowItem.label + rowIdx)"
    ...
    <el-pagination v-if="query && query.pageNo" @current-change="handleCurrentChange" @size-change="handleSizeChange"
      :current-page="query.pageNo" :page-size="query.pageSize" :page-sizes="[10, 20, 30]"
