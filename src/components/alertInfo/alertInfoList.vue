<template>
  <div>
    ...
    <div class="list-model">
      ...
      <alertInfoEdit v-if="drawer" :drawer="drawer" :rowData="rowData" @close="drawer = false" @success="success()" />
    </div>
  </div>
</template>

<script>
import alertInfoEdit from "./alertInfoEdit";
...
export default {
  ...
  methods: {
    search() {
      let params = {
        type: this.$route.params.type || "",//预警类型（0：连续未动销预警，1：满仓预警，2：缺货预警）
