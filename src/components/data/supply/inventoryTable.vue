<template>
  <div class="wrap-definition">
    ...
    <configEdit v-if="drawer" :drawer="drawer" :rowData="rowData" @close="drawer = false" @success="success()" :supplierInventoryList="supplierInventoryList"/>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.supplierKey=this.rowData.supplierKey
    console.log(this.row,this.row.supplierKey)
  ...
  methods: {
    ...
    deleteRow(row) {
        ...
        supplierInventoryDelete({ belongKey: this.supplierKey,inventoryKey:row.inventoryKey }).then(res => {
    ...
    getInventorylistPage() {
      inventorylistPage({ belongKey: this.supplierKey, page: this.pageNo, size: this.pageSize }).then(
        (res) => {
